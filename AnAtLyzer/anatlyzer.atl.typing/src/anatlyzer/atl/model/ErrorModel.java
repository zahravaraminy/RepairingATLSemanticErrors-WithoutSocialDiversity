...
public class ErrorModel {
	...
	private AnalysisResult	result;
	 //int nberrors=1;
	 int nberrors=0;
	  List<Integer> startrule = new ArrayList<Integer>();
	  List<Integer> finishrule = new ArrayList<Integer>();
	  List<String> faultlocationnofeature = new ArrayList<String>();
	  List<String> msgerror = new ArrayList<String>();
	 public static int error=0;
	 List<String> faultlocation = new ArrayList<String>();
	...
	public  List<String> getfaultlocation(){
		
		return this.faultlocation;
	}
	
public  List<String> getfaultlocationnofeature(){
		
		return this.faultlocationnofeature;
	}
...
public  List<String> getmsgerror(){
	...
	public Type signalNoFeature(EClass c, String featureName, LocatedElement element) {
		...
		this.faultlocationnofeature.add(element.getLocation());
		this.msgerror.add(featureName );
	...
	public Type signalNoOperationFound(Type receptorType, String operationName, LocatedElement node, IRecoveryAction ra) {
		...
		if ( ra != null ) {
			...
			if ( result != null ) {
				nberrors++;
				this.faultlocation.add(node.getLocation());
	...
	public void signalError(Problem p, String msg, LocatedElement l) {
		...
		if ( AnalyserContext.debugMode() )
		{
			...
			this.faultlocation.add(l.getLocation());
...
//bod
	...
	public void signalNoBindingForCompulsoryFeature(EStructuralFeature unboundCompulsoryFeature, OutPatternElement pe) {
		...
		if (!unboundCompulsoryFeature.getEContainingClass().getName().equals("Arc")  ) 
		{
			
			if( !pe.getLocation().contentEquals("48:1-56:2") && !pe.getLocation().contentEquals("49:1-57:2")
					&& !pe.getLocation().contentEquals("47:1-55:2")) {
			nberrors++;
			this.faultlocation.add(pe.getLocation());
	...
	public void signalBindingWithResolvedByIncompatibleRule(Binding b, EClass rightType, EClass targetType,
		...
		// in for baraye postprocessing hazf mishe vali baraye step aval proje bayad bashe 
		//if(! rightType.getName().equals("Classifier")) {
	/*	for (ResolvedRuleInfo rinfo : error.getRules()) {
			
			s += "\t" + rinfo.getRuleName() + " " + rinfo.getLocation() + "\n";
		//	if( !rinfo.getLocation().contentEquals("43:1-61:2") && !rinfo.getLocation().contentEquals("85:1-100:2") &&  !rinfo.getLocation().contentEquals("115:1-126:2")
			//		&& !rinfo.getLocation().contentEquals("41:1-54:2") && !rinfo.getLocation().contentEquals("28:1-41:2") && !rinfo.getLocation().contentEquals("43:1-56:2")
			//		&& !rinfo.getLocation().contentEquals("80:1-98:2") &&  !rinfo.getLocation().contentEquals("45:1-58:2") && 
			//		!rinfo.getLocation().contentEquals("30:1-43:2")&&  !rinfo.getLocation().contentEquals("60:1-78:2") && !rinfo.getLocation().contentEquals("29:1-42:2")
			//		&& !rinfo.getLocation().contentEquals("44:1-57:2") && !rinfo.getLocation().contentEquals("77:1-95:2") && !rinfo.getLocation().contentEquals("79:1-97:2")
				//	 && !rinfo.getLocation().contentEquals("48:1-61:2") && !rinfo.getLocation().contentEquals("86:1-101:2") && !rinfo.getLocation().contentEquals("116:1-127:2")) {
				if( rightType.getName().equals("Classifier")) {
					String[] array2 = b.getLocation().split(":", 2);
					if((Integer.parseInt(array2[0])>=ATLModel.startrule.get(3) && Integer.parseInt(array2[0])<=ATLModel.finishrule.get(3))
						||( Integer.parseInt(array2[0])>=ATLModel.startrule.get(4) && Integer.parseInt(array2[0])<=ATLModel.finishrule.get(4)))
								firstcheck=true;
					
				}
				if( rightType.getName().equals("NetContentElement")) {
					String[] array2 = b.getLocation().split(":", 2);
				...
				if( rightType.getName().equals("Attribute") 
						) {
					String[] array1 = b.getLocation().split(":", 2);
					
					if(Integer.parseInt(array1[0])>=ATLModel.startrule.get(1) && Integer.parseInt(array1[0])<=ATLModel.finishrule.get(1))
					{
						
						if(( thirdcheck==false && secondcheck==false &&  rinfo.getRuleName().equals("MultiValuedDataTypeAttribute2Column"))
							) {
					String[] array2 = rinfo.getLocation().split(":", 2);
					if( Integer.parseInt(array2[0])>=ATLModel.startrule.get(4) && Integer.parseInt(array2[0])<=ATLModel.finishrule.get(4)) {
						thirdcheck=true;
					}
					
					}
						if(( secondcheck==true &&  rinfo.getRuleName().equals("MultiValuedDataTypeAttribute2Column"))
								) {
						String[] array2 = rinfo.getLocation().split(":", 2);
						if( Integer.parseInt(array2[0])>=ATLModel.startrule.get(4) && Integer.parseInt(array2[0])<=ATLModel.finishrule.get(4)) {
							firstcheck=true;
						...
						if(secondcheck==false && thirdcheck==false &&( rinfo.getRuleName().equals("MultiValuedClassAttribute2Column"))) {
							String[] array2 = rinfo.getLocation().split(":", 2);
								if(( Integer.parseInt(array2[0])>=ATLModel.startrule.get(6) && Integer.parseInt(array2[0])<=ATLModel.finishrule.get(6))	)
								secondcheck=true;
						}
						if(thirdcheck==true && ( rinfo.getRuleName().equals("MultiValuedClassAttribute2Column"))) {
							String[] array2 = rinfo.getLocation().split(":", 2);
								if(( Integer.parseInt(array2[0])>=ATLModel.startrule.get(6) && Integer.parseInt(array2[0])<=ATLModel.finishrule.get(6))	)
								firstcheck=true;
						}
				...
				if(firstcheck==false) {
					...
					//this.faultlocation.add( rinfo.getLocation());
				location=rinfo.getLocation();
				}
		...
		//}
		if(firstcheck==false) {
	//		this.faultlocation.add( location);  for pnml2petrinet and class2relational case
	//		nberrors++;   for pnml2petrinet and class2relational case
		}
	...
	//	System.out.println("Error: Possibly");
	...
	public void signalBindingPrimitiveExpectedButObjectAssigned(Binding binding, Metaclass targetVar, String propertyName) {
		nberrors++;
		System.out.println("Error: In binding");
		this.faultlocation.add(binding.getLocation());
	...
	public static class NoRecoverableError extends RuntimeException {
		...
		public  NoRecoverableError(String msg) {		
 super(msg);	 }
	}
