 ...
 * @param csvWriterpareto 
   */       
  public Ranking(SolutionSet solutionSet, FileWriter csvWriterpareto) {        
   ...
   // NSGAII.writer.println("front aval");
    for (int p = 0; p < solutionSet_.size(); p++) {
      if (dominateMe[p] == 0) {
        front[0].add(p);
       // NSGAII.writer.println(p);
        solutionSet.get(p).setRank(0);
      }
    }    
 //   NSGAII.writer.println(front[0].size());
    //Obtain the rest of fronts
   
    int sizefront=1;
 //   writer3.print(front[0].size()+" ");
    int i = 0;
    Iterator<Integer> it1, it2 ; // Iterators
    while (front[i].size()!= 0) {
      i++;
      
   //   NSGAII.writer.println("next front");
      it1 = front[i-1].iterator();
      sizefront=sizefront+1;
    ...
    try {
		csvWriterpareto.append(sizefront-1+"\n");
	} catch (IOException e) {
